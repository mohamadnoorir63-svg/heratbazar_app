...
const String apiBase = 'https://api.kooktalayi.com/heratbazar-api/api';
const String publicBase = 'https://api.kooktalayi.com/heratbazar-api';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<List<dynamic>> fetchAds() async {
    final response = await http.get(Uri.parse('$apiBase/ads'));
  ...
  String fullImageUrl(String? url) {
    ...
    if (clean.startsWith('/')) return '$publicBase$clean';
  ...
  List<String> getImages(dynamic ad) {
    ...
    if (images is List) {
      for (final image in images) {
        final url = fullImageUrl(image.toString());
    ...
    final imageUrl = fullImageUrl(ad['image_url']?.toString());
  ...
  String getMainCategory(dynamic ad) {
    ...
    for (final line in description.split('\n')) {
      final t = line.trim();
      if (t.startsWith('دسته اصلی:')) {
        return t.replaceFirst('دسته اصلی:', '').trim();
      }
    }
  ...
  String getSubCategory(dynamic ad) {
    ...
    for (final line in description.split('\n')) {
      final t = line.trim();
      if (t.startsWith('زیر دسته:')) {
        return t.replaceFirst('زیر دسته:', '').trim();
      }
    }
  ...
  String cleanDescriptionPreview(dynamic ad) {
    ...
    return description
        ...
        .where((line) {
          ...
          if (t.startsWith('دسته اصلی:')) return false;
          if (t.startsWith('زیر دسته:')) return false;
          if (t.startsWith('ولایت:')) return false;
          if (t.startsWith('ولسوالی:')) return false;
          if (t.startsWith('مشخصات')) return false;
  ...
  void showAccountMenu() {
    showModalBottomSheet(
      ...
      builder: (_) {
        ...
        final name = AuthService.userFullName;
          ...
          child: SafeArea(
            child: Wrap(
              children: [
                ListTile(
                  leading: Icon(
                    logged ? Icons.account_circle : Icons.login,
                    color: Colors.deepPurple,
                  ),
                  title: Text(
                    logged ? (name.isEmpty ? 'حساب من' : name) : 'ورود به حساب',
                  ),
                  subtitle: Text(
                    logged ? phone : 'برای ثبت آگهی وارد شوید',
                  ),
                  onTap: () {
